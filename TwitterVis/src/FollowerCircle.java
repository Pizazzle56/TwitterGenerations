...
public class FollowerCircle {
	...
	public void draw(PApplet parentApplet) {
		for(int i = 0; i < fImage.size(); i++) {
			...
			fImage.get(i).setxPos(parentApplet.width/2 + parentApplet.cos((float) (circleChunk + 5 * i))*circleRadius);
			fImage.get(i).setyPos(parentApplet.height/2 + parentApplet.sin((float) circleChunk + 5 * i)*circleRadius);
